-- Atualiza função para incluir tiers atuais (start, prime, imob_*, const_*, fundador_*)
...
BEGIN
  ...
  v_max_auto_stories := CASE v_tier
    ...
    WHEN 'prime' THEN 3
    WHEN 'vip' THEN 4
    WHEN 'fundador_corretor' THEN 2
    -- Imobiliária
    WHEN 'imob_start' THEN 4
    WHEN 'imob_pro' THEN 6
    WHEN 'imob_elite' THEN 10
    -- Construtora
    WHEN 'const_start' THEN 6
    WHEN 'const_pro' THEN 10
    WHEN 'const_master' THEN 20
    -- Empresa legado
    WHEN 'essencial_empresa' THEN 8
    WHEN 'premium_empresa' THEN 10
    WHEN 'prime_empresa' THEN 20
    WHEN 'fundador_empresa' THEN 8
    ELSE 0
  END;
...
-- Anexa o trigger que estava faltando
DROP TRIGGER IF EXISTS trg_auto_create_story_on_new_item ON public.seller_items;
CREATE TRIGGER trg_auto_create_story_on_new_item
