-- 1. Adiciona billing_period em seller_subscriptions para distinguir Mensal/Anual/Fundador
ALTER TABLE public.seller_subscriptions
ADD COLUMN IF NOT EXISTS billing_period text NOT NULL DEFAULT 'monthly'
CHECK (billing_period IN ('monthly', 'annual', 'founder'));

-- 2. Atualiza grant_plan_credits para entregar todos os créditos de uma vez no Anual
...
DECLARE
  ...
  v_is_founder boolean := (p_billing_period = 'founder');
BEGIN
  ...
  -- Anual: entrega 12 meses de uma vez. Fundador e Mensal: entregam o valor base.
  IF v_is_annual THEN
  ...
  SET balance = balance + v_total,
      monthly_plan_credits = CASE WHEN v_is_annual OR v_is_founder THEN 0 ELSE v_monthly END,
  ...
  INSERT INTO public.ai_credit_transactions (
    ...
    CASE
      ...
      WHEN v_is_founder THEN 'Créditos Fundador entregues na compra'
...
-- 3. Atualiza refresh_ai_monthly_credits para NÃO recarregar Anual nem Fundador
...
BEGIN
  ...
  v_is_founder := v_tier IN ('fundador_corretor', 'fundador_empresa') OR v_billing_period = 'founder';
  ...
  -- Apenas Mensal recarrega. Anual e Fundador receberam tudo na compra.
  ...
  ELSE
    UPDATE public.ai_credit_wallets
    SET monthly_plan_credits = CASE WHEN v_is_founder OR v_is_annual THEN 0 ELSE v_monthly END,
        seller_id = COALESCE(seller_id, v_seller_id),
        updated_at = now()
    WHERE id = v_wallet_id
    RETURNING balance INTO v_balance;
  ...
  RETURN jsonb_build_object(
    'wallet_id', v_wallet_id,
    'balance', v_balance,
    'monthly_plan_credits', CASE WHEN v_is_founder OR v_is_annual THEN 0 ELSE v_monthly END,
    'tier', v_tier,
